...
| `rqunit conformance` | Manifest ↔ code surfaces (CF1–CF6) from each stack's committed `actual-surface.json`. Rust extractor: `cargo run -p spec-conformance-tests --bin extract-surface` (its cargo test proves the artifact is current). Ratified exceptions ride in the artifact and report as findings | After route/message changes; every gate |
