...
| `rqunit conformance` | Manifest ↔ code surfaces (CF1–CF9) from each stack's committed `actual-surface.json`. The extractor runs in the stack's own build (its test proves the artifact is current). Each artifact declares which surface families it examined; ratified exceptions live in `spec/framework/conformance-exceptions.yaml` — an extractor observes, and does not get to excuse what it observed — and still report as findings | After route/message changes; every gate |
