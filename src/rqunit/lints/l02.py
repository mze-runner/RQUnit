...
def _prose(text: str) -> str:
    """Blank every valid reference-token span (malformed tokens are L15's
    class and stay visible to the scan)."""
...
def run(store):
    ...
    for ru in store.rus():
        ...
        if ast is None:
            continue
        bound_prose = _prose(ast.bound.text) if ast.bound else ""
        response_prose = _prose(ast.response)
