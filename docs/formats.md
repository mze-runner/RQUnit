...
**Two kinds of thing are called an artifact.** A **store artifact** is an
addressable unit under `spec/` — an RU, an INT, a FEAT, a manifest, a model, a GAP,
an ADR, a packet — authored and reviewed, listed in the table below. `artifacts`
inside a manifest is a table of credential shapes (§16), part of a store
artifact's contents. An **adapter artifact** is a file a probe produced —
`actual-surface.json`, `scanned-checks.json`, `check-evidence.json` — declared as
`artifact = "path"` in `rqunit.toml` and generated by your pipeline. This
reference says "store artifact" or names the file wherever a sentence could mean
either.
...
**Write `inbound: none` for a surface that accepts nothing.** `inbound: { fields:
none }` asserts the same thing and additionally carries an `unknown_fields`
policy; the slot-level form is the standard spelling. An outbound shape always
declares its `status`, so a body-less response is `{ status: 204, fields: none }`.
...
scaffolded the same way: `rqunit init` writes it empty. An empty registry and an
absent file mean the same thing — the store has no segments and its ids carry
none, which is a complete state rather than an unfinished one. Declare a segment
when the store has a domain to allocate into; a segment name is permanent once
its first id is minted.
...
artifacts:
  ...
  refresh-token:
    access_tier: refresh
    fields: none                      # opaque token: nothing to describe inside it
```

**The access tier binds a surface to the credential that admits it.** Every tier a
declared surface uses resolves to exactly one artifact carrying that `access_tier`,
or is listed in `credential_free_tiers` (C17). An endpoint names its tier and
nothing else — the tier string is the join, and the credential is derived from it.
A tier admits one credential shape; two accepted shapes are two tiers.

An artifact does two jobs: it names the credential a tier requires, and it
describes that credential's internals where they can be described. `fields: none`
does the first alone, which is what an opaque token needs. Presence uses the
outbound vocabulary (`always | never`) because a credential is minted rather than
accepted, and C11 checks this census like every other.

Fields use the same grammar as every surface census (§13), so `fields: none` means
here what it means everywhere: this carries nothing, stated rather than left
unfinished. The one addition is `where` (`claims | header`), which places a field
inside the encoded structure. `where` is legal on an artifact census and nowhere
else — on a payload the position is the field name.
...
**Artifacts live in the shared manifest whoever consumes them** — one registry,
one place to look for what a credential is, including a credential only one
service accepts. A service manifest carries no `artifacts` table, and
promotion-by-reuse does not apply: there is nowhere else for one to start.
